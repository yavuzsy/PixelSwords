...
public class PlayerCntrl1 : MonoBehaviourPunCallbacks
{
    ...
    public DynamicJoystick joystick1;
    ...
    private void Update()
    {
        ...
        horizontalInput = Input.GetAxis("Player1H");
        verticalInput = Input.GetAxis("Player1V");*/
        horizontalInput = joystick1.Horizontal;
        verticalInput = joystick1.Vertical;
        ...
        // Saldırı kontrolü
        if (Input.GetKeyDown(KeyCode.Space) && attackOn)
        {
            animator.SetBool("SwordAttack", true);
        }
        else if (Input.GetKeyUp(KeyCode.Space) && attackOn)
        {
            ...
            foreach (Collider2D collider in colliders)
            {
                if (collider.CompareTag("Player2"))
                {
                    ...
                    collider.GetComponent<PlayerCntrl2>().TakeDamage(1, damageDirection);
                }
            }
    ...
    /*public void TakeDamage(float damageAmount, Vector2 directional)
    {
        ...
        if (health <= 0)
        {
            // Eğer canı 0 veya daha az ise oyuncu yok olsun (destroy)
            StartCoroutine(ChangeColorCoroutine());
            manager.GetComponent<GameManager>().winner(winp2);
            //Destroy(gameObject, 0.52f);
            PhotonNetwork.Destroy(gameObject);
        }
        else
        {
            rb.AddForce(directional * 50f,ForceMode2D.Impulse);
            StartCoroutine(sekme());
            // Eğer canı 0'dan büyük ise belirli bir süre boyunca renk değişimi yapabiliriz
            StartCoroutine(ChangeColorCoroutine());
        }
    }*/
    [PunRPC]
